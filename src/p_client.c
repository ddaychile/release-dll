...
cvar_t* exbattleinfo = NULL; //hans inicializa exbattleinfo
...
void KillingSpree(edict_t* attacker, edict_t* self, edict_t* Tent)
{
	...
	if (self->client->resp.streak >= exbattleinfo->value &&
		attacker && attacker->client && attacker != self) // Verifica que attacker no sea NULL
	{
		for (i = 1; i <= game.maxclients; i++)
		{
			Tent = &g_edicts[i];
			...
			safe_cprintf(Tent, PRINT_MEDIUM,
				"** %s has ENDED %s KILLING SPREE. Total frags: %d **\n",
				attacker->client->pers.netname,
				self->client->pers.netname,
				self->client->resp.streak);
		}
	}

	if (self->client->resp.streak >= exbattleinfo->value && 
		attacker && attacker->client && attacker != self) // // Verifica que attacker no sea NULL, Si el jugador está en racha y cambia de equipo 0 se suicida se anuncia su error :P
		for (i = 1; i <= game.maxclients; i++)
		{
			Tent = &g_edicts[i];
				if (!Tent->inuse || !Tent->client)
...
void Set_Stance_Max(edict_t *ent)
{
	...
	else if (ent->s.frame == 283) ent->stance_max = 24;
